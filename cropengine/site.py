...
class WOFOSTSiteParametersProvider(dict):
    ...
    def __init__(self, profile, **kwargs):
        ...
        if profile not in config['profiles']:
            raise SiteParameterError(f"Unknown profile '{profile}'. Available profiles: {list(config['profiles'].keys())}")
        ...
        self.param_metadata = {param:all_param_defs[param] for param in self.valid_param_names}
        ...
        for par_name in self.valid_param_names:
            ...
            if par_name in kwargs:
                ...
                if par_name in self.required_params:
                    raise SiteParameterError(f"Value for parameter '{par_name}' is required for profile '{profile}'!")
    ...
    def _convert_and_validate(self, name, value, definition):
        ...
        try:
            if target_type_str == 'int':
             ...
             raise SiteParameterError(f"Parameter '{name}' must be of type {target_type_str}, got {type(value)}")
        ...
        if target_type_str == 'list':
             ...
             if not all(min_val <= x <= max_val for x in value):
                  raise SiteParameterError(f"Elements in list '{name}' must be between {min_val} and {max_val}")
            ...
            if not (min_val <= value <= max_val):
                 raise SiteParameterError(f"Value {value} for parameter '{name}' out of range [{min_val}, {max_val}]")

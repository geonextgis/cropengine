...
class WOFOSTSensitivityAnalyzer:
    ...
    def run_analysis(self, 
        ...
        if method == 'sobol':
            param_values = saltelli.sample(problem_dict, n_samples, calc_second_order=False)
        elif method == 'morris':
            param_values = morris_sampler.sample(problem_dict, n_samples, num_levels=num_levels)
        elif method == 'fast':
            param_values = fast_sampler.sample(problem_dict, n_samples)
        else:
            raise ValueError(f"Unknown method: {method}. Available methods are: {'sobol', 'morris', 'fast'}.")
        ...
        print(f"[SA] Mode: {mode.upper()} | Params: {total_runs_per_loc} | Locations: {len(selected_locs)}")
        ...
        if mode == 'global':
            ...
            for loc_id in selected_locs:
                ...
                df_loc = self._analyze_results(problem_dict, param_values, y_local, method)

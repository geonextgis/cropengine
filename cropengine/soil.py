...
class GEEIsricSoilDataProvider:
    ...
    ASSET_CONFIG = {
        'bdod': {'asset': 'projects/soilgrids-isric/bdod_mean', 'mapped_unit': 'cg/cm³', 'factor': 100, 'transformed_unit': 'kg/dm³'},
        'cec':  {'asset': 'projects/soilgrids-isric/cec_mean', 'mapped_unit': 'mmol(c)/kg', 'factor': 10,  'transformed_unit': 'cmol(c)/kg'},
        'cfvo': {'asset': 'projects/soilgrids-isric/cfvo_mean', 'mapped_unit': 'cm³/dm³', 'factor': 10,  'transformed_unit': 'cm³/100cm³'},
        'clay': {'asset': 'projects/soilgrids-isric/clay_mean', 'mapped_unit': 'g/kg', 'factor': 10,  'transformed_unit': '%'},
        'sand': {'asset': 'projects/soilgrids-isric/sand_mean', 'mapped_unit': 'g/kg', 'factor': 10,  'transformed_unit': '%'},
        'silt': {'asset': 'projects/soilgrids-isric/silt_mean', 'mapped_unit': 'g/kg', 'factor': 10,  'transformed_unit': '%'},
        'nitrogen': {'asset': 'projects/soilgrids-isric/nitrogen_mean', 'mapped_unit': 'cg/kg', 'factor': 100, 'transformed_unit': 'g/kg'},
        'phh2o': {'asset': 'projects/soilgrids-isric/phh2o_mean', 'mapped_unit': 'pH*10', 'factor': 10, 'transformed_unit': 'pH'},
        'soc':   {'asset': 'projects/soilgrids-isric/soc_mean', 'mapped_unit': 'dg/kg', 'factor': 10, 'transformed_unit': 'g/kg'},
        'ocd':   {'asset': 'projects/soilgrids-isric/ocd_mean', 'mapped_unit': 'hg/m³', 'factor': 10, 'transformed_unit': 'kg/m³'}
    }
    ...
    def __init__(
        ...
        if properties is None:
            ...
            if not self.properties:
                raise ValueError(f"No valid properties found in input. Available: {list(self.ASSET_CONFIG.keys())}")
    ...
    def _construct_image(self):
        ...
        if not images_to_merge:
            raise ValueError("Could not construct image from selected properties/depths.")
...
class WOFOSTSoilParameterProvider:
    ...
    # Default parameters for potential production
    _defaults = {
        "SMFCF": 0.3, "SM0": 0.45, "SMW": 0.1,
        "RDMSOL": 120, "CRAIRC": 0.06, "K0": 10.0,
        "SOPE": 10.0, "KSUB": 10.0,
        "Soil_pH": 7.0, "FSOMI": 0.01, "CNRatioSOMI": 10.0
    }
    ...
    def _get_val(self, prop_name):
        """Helper to safely extract a specific property value from the DataFrame."""
        try:
            return self.df.loc[self.df["property"] == prop_name, "transformed_value"].values[0]
        except IndexError:
            raise ValueError(f"Missing required property: {prop_name}")
    ...
    def get_params(self):
        ...
        if th_10 is not None and th_33 is not None and th_1500 is not None:
            ...
            try:
                # Fit Van Genuchten
                popt, _ = curve_fit(
                    lambda h, tr, a, n: self._van_genuchten_theta(h, tr, a, n, theta_s=current_sm0),
                    np.array(h_obs),
                    np.array(th_obs),
                    p0=[0.01, 0.01, 1.5],
                    bounds=bounds,
                ...
                self.params["CRAIRC"] = float(round(max(0.05, self.params['SM0'] - self.params["SMFCF"]), 3))           

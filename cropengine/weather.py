...
class ClimateConfig:
    ...
    def _parse_variables(self):
        for var_name, info in self.variables.items():
            self.var_to_units[var_name] = (info.get("native_unit"), info.get("target_unit"))
...
class GEEWeatherDataProvider:
    ...
    def __init__(self, start_date, end_date, 
                 latitude=None, longitude=None, geometry=None, 
                 source='era5_land', filepath=None, ee_project=None, **site_kwargs):
        ...
        if geometry:
            ...
            raise ValueError("Must provide either 'geometry' OR 'latitude' and 'longitude'.")
        ...
        if source.lower() not in full_config:
            raise ValueError(f"Source '{source}' not found. Available: {list(full_config.keys())}")
    ...
    def _get_elevation(self):
        ...
        try:
            geom = self.region
            dem_source = self.weather_config.get('dem_source', "projects/sat-io/open-datasets/GLO-30")

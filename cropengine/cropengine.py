...
from cropengine.soil import IsricSoilDataProvider, GEEIsricSoilDataProvider, WOFOSTSoilParameterProvider
from cropengine.site import WOFOSTSiteParametersProvider
from cropengine.crop import WOFOSTCropParametersProvider
from cropengine.agromanagement import WOFOSTAgroEventBuilder, WOFOSTAgroManagementProvider
...
class WOFOSTCropSimulationRunner:
    ...
    def prepare_system(self, 
                       ...
                       crop_start, crop_end,
                       model_name, crop_name, variety_name,
                       crop_start_type="emergence", crop_end_type="harvest", max_duration=300,
                       timed_events=None, state_events=None,
                       force_update=False, **site_kwargs):
        """
        ...
        self._ensure_weather(latitude, longitude, campaign_start, campaign_end, force_update)
        ...
        # 3. Crop
        self._save_params(WOFOSTCropParametersProvider(crop_name, variety_name), "crop_params")
        ...
        self._build_agromanagement(
            ...
            max_duration, timed_events, state_events
        )
    ...
    def run_simulation(self, 
        ...
        if agro_file_path:
            if not os.path.exists(agro_file_path):
                raise FileNotFoundError(f"Custom agromanagement file not found: {agro_file_path}")
            agro_path = agro_file_path
            print(f"[RUN] Using custom agromanagement: {agro_path}")
        else:
            if not os.path.exists(self.files['agro']):
                raise FileNotFoundError("Default agromanagement file missing. Run 'prepare_system' or provide 'custom_agro_file'.")
            agro_path = self.files['agro']
            
        # 1. Load Weather (Must exist)
        if not os.path.exists(self.files['weather']) or not os.path.exists(self.files['agro']):
            raise FileNotFoundError("Missing weather or agro files. Run 'prepare_system' first.")
    ...
    def _ensure_weather(self, lat, lon, start, end, force):
        if not force and os.path.exists(self.files['weather']): return
        
        d_end = (datetime.strptime(end, "%Y-%m-%d").date() + timedelta(days=1)).strftime("%Y-%m-%d")
        meteo = GEEWeatherDataProvider(start_date=start, end_date=d_end, latitude=lat, longitude=lon, filepath=self.files['weather'])
        meteo.save_weather_excel()

    def _ensure_soil(self, lat, lon, force):
        if not force and os.path.exists(self.files['soil']): return self.files['soil']
        
        soil = GEEIsricSoilDataProvider(latitude=lat, longitude=lon, depths=["0-5cm"], filepath=self.files['soil'])
        soil.get_data()
        return self.files['soil']

    def _build_agromanagement(self, c_start, c_end, start, end, crop, var, 
                              st_type, end_type, dur, timed_events, state_events):
        agro = WOFOSTAgroManagementProvider()
        agro.add_campaign(c_start, c_end, crop, var, start, end, st_type, end_type, dur, timed_events, state_events)

...
class WOFOSTOptimizer:
    ...
    def optimize(self, 
                 ...
                 output_folder: Optional[str] = None) -> optuna.Study:
        """
        ...
        def objective(trial):
            ...
            tasks = [
                (loc_id, engine, overrides) 
                for loc_id, engine in self.engines.items()
            ]
            ...
            with ProcessPoolExecutor(max_workers=n_workers) as executor:
                ...
                for res in executor.map(_global_worker_task, tasks, chunksize=chunk_size):
                    if res is not None:
                        results.append(res)
        ...
        print(f"[OPT] Starting {len(directions)}-objective optimization with {n_trials} trials...")
        ...
        if len(directions) == 1:
            ...
            print(f"Pareto front found with {len(study.best_trials)} optimal solutions.")
